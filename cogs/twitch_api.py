...
#Cogs.
class TWITCHAPI:
    """Helix with auto token refresh and a shared session."""
    def __init__(self):
    ...
    async def _ensure_token(self):
        if self.token and time.time() < self.token_expiry_ts - 60:
            return
        sess = await self.session.close()
        async with sess.post("https://id.twitch.tv/oauth2/token", data = {"client_id": TWITCH_CLIENT, 
        ...
        async def fetch_streams(self, logins: List[str]):
                if not logins:
                        return []
        ...
        async def fetch_clips(self, broadcaster_id: str, started_at_iso: str):
                sess = await self._get_session()
